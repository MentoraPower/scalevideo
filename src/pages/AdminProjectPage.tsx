...
export default function AdminProjectPage() {
  ...
  const uploadUrl = `${origin}/upload/${project.slug}`
  const viewUrl = `${origin}/view/${project.slug}`
    ...
    <div className="app">
      <main className="main main-wide">
        ...
        <header className="page-header">
          <h1>{project.name}</h1>
          <p className="subtitle">/{project.slug}</p>
        </header>
        ...
        <section className="card">
          ...
          <div className="link-row">
            <div className="link-info">
              <span className="link-label">Link do cliente (subir vídeo)</span>
              <a href={uploadUrl} target="_blank" rel="noreferrer" className="link-url">
                {uploadUrl}
              </a>
            </div>
            <button
              ...
              onClick={() => copy(uploadUrl, 'up')}
            >
              {copied === 'up' ? 'Copiado' : 'Copiar'}
            </button>
          </div>

          <div className="link-row" style={{ marginTop: 8 }}>
            <div className="link-info">
              <span className="link-label">Link do editor (baixar / editar)</span>
              <a href={viewUrl} target="_blank" rel="noreferrer" className="link-url">
                {viewUrl}
              </a>
            </div>
            <button
              type="button"
              className="ghost copy-btn"
              onClick={() => copy(viewUrl, 'vw')}
            >
              {copied === 'vw' ? 'Copiado' : 'Copiar'}
            </button>
          </div>
        </section>
                  ...
                  <li key={v.id} className="video-item view">
                    ...
                    {isEditing ? (
                      ...
                      <div className="video-info">
                        ...
                        <span className="video-meta">{formatBytes(v.size_bytes ?? 0)}</span>
                      </div>
                    )}
